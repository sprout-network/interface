import { platformTypeHandler, safeUserImage, timeConverHandler, truncatedText } from "@/core/utils";
...
async function handleMint(
  ...
  if (chain === undefined) {
    ...
    alert(`Please switch to the ${APP_CHAIN}.`);
  } else {
    if (metadata) {
      try {
...
const PreviewTable: FC<PreviewTableProps> = ({
  ...
  if (!connected) {
    ...
    return (
      <div className="col-span-4">
        <div className="fixed top-[165px] w-96 backdrop-blur-md shadow-md bg-white/20 rounded-lg 
        flex flex-col gap-4 px-10 py-4">
          ...
          <main className="flex flex-col gap-3">
            <div className="text-gray-400">Bio: {truncatedText(bio, 15)}</div>
